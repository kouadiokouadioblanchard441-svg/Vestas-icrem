...
export default function AdminCompanyContent() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: editingId ? "Bloc modifié" : "Bloc ajouté" });
      setShowForm(false);
      setEditingId(null);
      setForm(emptyForm);
    },
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <p className="text-sm text-muted-foreground">
          Ajoutez les informations de l’entreprise, les plans d’investissement et les images visibles dans « Compagnie ».
        </p>
      ...
      {showForm && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-base">{editingId ? "Modifier le bloc" : "Nouveau bloc"}</CardTitle>
            <Button variant="ghost" size="icon" onClick={() => setShowForm(false)} aria-label="Fermer">
              <X className="w-4 h-4" />
            </Button>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="company-content-title">Titre</Label>
              <Input
                id="company-content-title"
                value={form.title}
                onChange={(event) => updateForm("title", event.target.value)}
                placeholder="Ex. Notre entreprise"
                data-testid="input-company-content-title"
              />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="company-content-body">Texte</Label>
              <Textarea
                id="company-content-body"
                value={form.body}
                onChange={(event) => updateForm("body", event.target.value)}
                rows={6}
                placeholder="Présentez l’entreprise, ses activités ou un plan d’investissement..."
                data-testid="input-company-content-body"
              />
            </div>
            <div className="space-y-2">
              <Label>Image <span className="font-normal text-muted-foreground">(optionnelle)</span></Label>
              {form.imageUrl && (
                <div className="relative h-32 rounded-xl border overflow-hidden bg-secondary/30">
                  <img src={form.imageUrl} alt="Aperçu" className="w-full h-full object-contain" />
                  <button
                    type="button"
              ...
              <Button type="button" variant="outline" className="w-full" onClick={() => document.getElementById("company-content-image")?.click()}>
                <ImagePlus className="w-4 h-4 mr-2" /> {form.imageUrl ? "Changer l’image" : "Choisir une image"}
              </Button>
              <Input
            ...
            <div className="grid grid-cols-2 gap-4 items-end">
              <div className="space-y-1.5">
                <Label htmlFor="company-content-order">Ordre d’affichage</Label>
      ...
      {isLoading ? (
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Ordre {block.sortOrder} · {block.isActive ? "Visible" : "Masqué"}</p>
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      <Switch checked={block.isActive} onCheckedChange={(isActive) => toggleMutation.mutate({ id: block.id, isActive })} />
                      <Button size="icon" variant="ghost" onClick={() => startEdit(block)} aria-label="Modifier">
                      ...
                      <Button
                        ...
                        onClick={() => { if (confirm(`Supprimer « ${block.title} » ?`)) deleteMutation.mutate(block.id); }}
                  ...
                  <p className="mt-2 text-sm text-muted-foreground line-clamp-2 whitespace-pre-line">{block.body || "Aucun texte"}</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="text-center py-8 text-muted-foreground">Aucun contenu. Ajoutez votre premier bloc.</div>
      )}
    </div>
  );
}