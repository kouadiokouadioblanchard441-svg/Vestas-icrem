...
There is no webhook-attempt logging table (checked `admin_audit_log` and full table list — nothing logs raw inbound webhook calls, only admin actions). So the DB cannot show whether WestPay actually attempted a callback for a given deposit; only whether a deposit ended up "processing" (never confirmed) vs "approved". To diagnose a real deposit that stays stuck in "processing", the WestPay merchant dashboard's own webhook delivery/callback log is the only way to see if WestPay tried to call our server at all, and what response it got.
