...
- Re-imported project (fifth time): ran `npm install` (tsx was missing), pushed DB schema with `npm run db:push -- --force` against the built-in Replit PostgreSQL (`DATABASE_URL`), restarted `Start application` workflow — server seeded fresh data and serves on port 5000. Login page confirmed rendering correctly.
